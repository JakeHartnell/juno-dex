...
class FakeSnapshotClient {
    queries = [];
    jobRows = [{ id: "1", chain_id: "juno-1", pair_address: "juno1pair", height: "39381355", block_time: "2026-07-01T03:01:00Z", reason: "touched", status: "leased", attempts: 1 }];
    async query(text, values) {
        this.queries.push({ text, values });
        if (text.includes("WITH claimable"))
            return { rows: this.jobRows, rowCount: this.jobRows.length };
        if (text.includes("FROM pools") && text.includes("pair_address"))
            return { rows: [{ id: "pool-1" }], rowCount: 1 };
...
describe("SnapshotWorker", () => {
    it("processes a claimed job by querying LCD at height, writing a snapshot, and marking success", async () => {
        ...
        const rest = {
            poolState: async (pairAddress, height) => {
                expect(pairAddress).toBe("juno1pair");
                expect(height).toBe(39381355);
                return { reserves: [{ denom: "ujuno", amount: "123" }], totalShare: "456" };
            },
        };
        await expect(new SnapshotWorker(config, pool, rest, { batchSize: 10, leaseSeconds: 30, maxAttempts: 3 }).processBatch()).resolves.toBe(1);
    ...
    it("retries transient LCD failures by returning the job to pending", async () => {
        ...
        await expect(new SnapshotWorker(config, pool, rest, { maxAttempts: 5 }).processBatch()).resolves.toBe(1);
