...
class FakeDb {
    ...
    async query(text, values) {
        ...
        if (text.includes("FROM schema_migrations"))
            return { rows: [{ version: "001_init.sql" }, { version: "002_pool_candles.sql" }, { version: "003_api_pricing_readiness.sql" }] };
        if (text.includes("FROM indexer_cursors"))
            return { rows: [{ last_height: "42", updated_at: "2026-07-03T00:00:00.000Z" }] };
        if (text.includes("FROM protocol_stats_latest"))
            return { rows: [{ pool_count: 1, incentivized_pools: 1, updated_at: "2026-07-03T00:00:00.000Z", tvl_usd: null, tvl_juno: "1000", volume_24h_usd: null, volume_24h_juno: "25", volume_7d_usd: null, volume_7d_juno: "100", fees_24h_usd: null, fees_24h_juno: "0.3" }] };
        if (text.includes("FROM token_prices"))
            return { rows: [{ asset: "ujuno", price_usd: null, price_juno: "1", source: "pool", status: "fresh", observed_at: "2026-07-03T00:00:00.000Z" }] };
        if (text.includes("FROM latest_pool_state") && text.includes("LIMIT 1")) {
            return { rows: [poolRow()] };
        }
        if (text.includes("FROM latest_pool_state"))
            return { rows: [poolRow()] };
        if (text.includes("FROM pools p") && text.includes("LIMIT 1"))
            return { rows: [poolRow()] };
        if (text.includes("FROM pools p"))
            return { rows: [poolRow()] };
...
async function start(db = new FakeDb()) {
    const store = new PostgresApiStore(db, "juno-1", "cursor");
...
describe("production API", () => {
    ...
    it("serves in-process ingestion throughput metrics when a collector is attached", async () => {
        const store = new PostgresApiStore(new FakeDb(), "juno-1", "cursor");
    ...
    it("uses one shared RPC head check per metrics scrape", async () => {
        ...
        const store = new PostgresApiStore(new FakeDb(), "juno-1", "cursor", { rpcUrl: "http://rpc.example", confirmationDepth: 2 });
        const server = createIndexerApi(store);
        await new Promise((resolve) => server.listen(0, resolve));
        openServer = server;
        const address = server.address();
    ...
    it("returns honest empty API responses when read models have no production rows", async () => {
        const { db, server, baseUrl } = await start(new EmptyReadModelDb());
        openServer = server;
        const stats = await (await fetch(`${baseUrl}/stats`)).json();
    ...
    it("returns HTTP 503 when readiness checks report not_ready", async () => {
        const db = new FakeDb();
        const store = new PostgresApiStore(db, "juno-1", "cursor", { expectedMigrationVersions: ["001_init.sql", "002_pool_candles.sql", "003_api_pricing_readiness.sql", "004_pool_state_source_precedence.sql"] });
        const server = createIndexerApi(store);
        await new Promise((resolve) => server.listen(0, resolve));
        openServer = server;
        const address = server.address();
    ...
    it("returns structured errors without leaking database internals", async () => {
        ...
        const { server, baseUrl } = await start(db);
        openServer = server;
