...
class FakeBlockClient {
    ...
    async query(text, values) {
        ...
        if (text.includes("FROM processed_blocks") && text.includes("height = $2 - 1")) {
            const rows = (this.rowsByKey.get(`previous:${String(values?.[1])}`) ?? []);
            return { rows, rowCount: rows.length };
        }
        if (text.includes("FROM processed_blocks") && text.includes("height = $2")) {
            const rows = (this.rowsByKey.get(`existing:${String(values?.[1])}`) ?? []);
            return { rows, rowCount: rows.length };
        }
        if (text.includes("INSERT INTO processed_blocks"))
            return { rows: [], rowCount: this.nextWriteRowCount };
        ...
        if (text.includes("WITH claimable"))
            return { rows: [{ id: "7", chain_id: "juno-1", pair_address: "juno1pair", height: "39381355", block_time: "2026-07-01T03:01:00Z", reason: "touched", status: "leased", attempts: 1 }], rowCount: 1 };
        if (text.includes("UPDATE snapshot_jobs"))
            return { rows: [], rowCount: 1 };
        if (text.includes("FROM pools") && text.includes("pair_address")) {
            const rows = (this.rowsByKey.get(`pool:${String(values?.[1])}`) ?? []);
            return { rows, rowCount: rows.length };
        }
        return { rows: [], rowCount: 1 };
    }
...
class FakeCandleClient {
    ...
    poolRows;
    queries = [];
    constructor(metadataRows = [{ asset: "ujuno", decimals: 6 }, { asset: "factory/token18", decimals: 18 }], swapRow = { pair_address: "juno1pair", block_time: "2026-07-01T03:01:00Z", offer_asset: "factory/backfill18", offer_amount: "2000000000000000000", ask_asset: "ujuno-backfill", return_amount: "3000000", height: "39381355", tx_hash: "tx", msg_index: "0", event_index: "0" }, poolRows = [{ id: "pool-1", pair_address: "juno1pair" }]) {
    ...
    async query(text, values) {
        ...
        if (text.includes("INSERT INTO swaps"))
            return { rows: [{ id: "swap-1", pool_id: values?.[1] ?? null }], rowCount: 1 };
        if (text.includes("INSERT INTO candle_jobs"))
            return { rows: [], rowCount: 1 };
        if (text.includes("WITH next_job"))
            return { rows: [{ id: "job-1", chain_id: "juno-1", pair_address: "juno1pair", from_time: "2026-07-01T00:00:00.000Z", to_time: "2026-07-02T00:00:00.000Z", attempts: 1, worker_id: values?.[1] }], rowCount: 1 };
        if (text.includes("UPDATE candle_jobs"))
            return { rows: [], rowCount: 1 };
        if (text.includes("FROM swaps"))
            return { rows: [this.swapRow], rowCount: 1 };
...
class FakeStageClient {
    ...
    async query(text, values) {
        this.queries.push({ text, values });
        if (text.includes("FROM processed_blocks") && text.includes("height = $2")) {
            const rows = this.previousBlockHash ? [{ block_hash: this.previousBlockHash }] : [];
            return { rows, rowCount: rows.length };
        }
        if (text.includes("INSERT INTO processed_blocks"))
            return { rows: [], rowCount: this.processedBlockRowCount };
        return { rows: [], rowCount: 1 };
    }
}
describe("migration runner", () => {
    ...
    it("records migrations once and skips already-applied files on subsequent runs", async () => {
        ...
        await expect(runMigrations(pool, dir)).resolves.toEqual(["001_init.sql", "002_next.sql"]);
        ...
        pool.queries = [];
        await expect(runMigrations(pool, dir)).resolves.toEqual([]);
...
describe("processed block recording", () => {
    it("rejects a conflicting block hash for an already processed height", async () => {
        ...
        await expect(recordProcessedBlock(client, {
            chainId: "juno-1",
            height: 39381305,
            blockHash: "new-hash",
            parentHash: "parent",
...
describe("bulk staging merge writer", () => {
    ...
    it("does not advance the cursor when a staging merge detects a processed block conflict", async () => {
        ...
        await expect(stageAndMergeBatch(client, {
    ...
    it("rejects non-contiguous or forked staged block ranges before advancing the cursor", async () => {
        ...
        await expect(stageAndMergeBatch(previousClient, {
...
describe("swap candle writes", () => {
    it("uses asset_metadata decimals for indexer candle price and volume math", async () => {
        ...
        await writeNormalizedEvent(client, "juno-1", {
            kind: "swap",
            chainId: "juno-1",
            height: 39381355,
            blockTime: "2026-07-01T03:01:00Z",
    ...
    it("uses decimals for candle backfills and skips degraded metadata", async () => {
        ...
        await expect(backfillTokenCandles(okClient, { chainId: "juno-1" })).resolves.toBe(1);
    ...
    it("worker claims a candle job, rebuilds candles through shared helper, and marks completion", async () => {
        ...
        await expect(processNextCandleJob(client, { chainId: "juno-1", workerId: "worker-1" })).resolves.toMatchObject({
            id: "job-1",
            pairAddress: "juno1pair",
        });
    ...
    it("writes pool discovery before same-batch pair events regardless of emitted order", async () => {
        ...
        await writeNormalizedEvents(client, "juno-1", [
            {
                kind: "swap",
                chainId: "juno-1",
                height: 39381355,
...
describe("pool state snapshots", () => {
    it("enqueues reserve snapshot jobs idempotently for known pools only", async () => {
        const client = new FakeBlockClient();
        await expect(enqueueSnapshotJobs(client, {
            chainId: "juno-1",
    ...
    it("claims snapshot jobs with skip-locked leases and updates terminal state", async () => {
        const client = new FakeBlockClient();
        await expect(claimSnapshotJobs(client, { chainId: "juno-1", limit: 10, leaseSeconds: 30, maxAttempts: 5 })).resolves.toEqual([
        ...
        await markSnapshotJobSucceeded(client, { jobId: "7", attempt: 1 });
        await markSnapshotJobFailed(client, { jobId: "8", attempt: 2, error: "temporary", permanent: false, maxAttempts: 5 });
    ...
    it("rejects snapshots for unknown pools instead of writing orphan state", async () => {
        const client = new FakeBlockClient();
        await expect(upsertPoolStateSnapshot(client, {
            chainId: "juno-1",
...
describe("API read model refresh", () => {
    it("calls the SQL refresh helper and maps affected rows", async () => {
        const client = {
            ...
            async query(text, values) {
                this.queries.push({ text, values });
                return { rows: [{ model: "latest_pool_state", rows_affected: "1" }, { model: "protocol_stats_latest", rows_affected: 1 }], rowCount: 2 };
            },
        };
        await expect(refreshApiReadModels(client, { chainId: "juno-1" })).resolves.toEqual([
