...
describe("JunoRestClient", () => {
    it("queries pair pool state at an explicit historical height", async () => {
        ...
        expect(init.headers).toMatchObject({ "x-cosmos-block-height": "39381355" });
    });
    it("rejects malformed pool responses instead of fabricating reserves", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValue({ ok: true, json: async () => ({ data: { assets: [] } }) });
...
describe("JunoRpcClient", () => {
    ...
    it("does not retry permanent RPC statuses and fails clearly", async () => {
        ...
        await expect(new JunoRpcClient("https://rpc.example", { maxRetries: 3 }).head()).rejects.toThrow("RPC /status failed: 400 Bad Request");
        expect(fetchSpy).toHaveBeenCalledTimes(1);
    });
    it("fails clearly after transient statuses exhaust retries", async () => {
        const fetchSpy = vi.spyOn(globalThis, "fetch")
            ...
            .mockResolvedValueOnce(response(503, "Service Unavailable", {}));
        await expect(new JunoRpcClient("https://rpc.example", { maxRetries: 1 }).head()).rejects.toThrow("RPC /status failed: 503 Service Unavailable");
        expect(fetchSpy).toHaveBeenCalledTimes(2);
    });
    ...
    it("retries transient failures from block result fetches", async () => {
        ...
        const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
            ...
            if (url === "https://rpc.example/block?height=9")
                return response(200, "OK", blockResponse(9));
            if (url === "https://rpc.example/block_results?height=9") {
                blockResultsAttempts += 1;
                if (blockResultsAttempts === 1)
    ...
    it("records fetched blocks only after both block RPC responses succeed", async () => {
        ...
        await expect(new JunoRpcClient("https://rpc.example", { maxRetries: 0, metrics }).block(9)).rejects.toThrow("RPC /block_results?height=9 failed: 500 Internal Server Error");
        expect(metrics.snapshot().fetchBlocksTotal).toBe(0);
    });
});
function blockResponse(height) {
    return {
        result: {
