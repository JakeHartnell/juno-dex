...
try {
    ...
    const diagnostics = await client.query(`WITH selected_swaps AS (
     ...
     SELECT
       ...
       (SELECT count(*) FROM token_candles WHERE chain_id = $1 AND ($2::text IS NULL OR pair_address = $2) AND ($3::timestamptz IS NULL OR bucket_start >= $3) AND ($4::timestamptz IS NULL OR bucket_start <= $4))::text AS token_candle_count`, [chainId, pairAddress ?? null, from ?? null, to ?? null]);
    ...
    if (stats) {
        ...
        if (stats.missing_assets?.length)
            console.log(`candle diagnostics missing_or_invalid_decimals=${stats.missing_assets.join(",")}`);
    }
}
finally {
    client.release();
