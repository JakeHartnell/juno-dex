export function attributesToRecord(attributes) {
    const out = {};
...
function parseAssets(raw) {
    ...
    if (denoms.length === 0 && amounts.length === 0)
        return [];
    if (denoms.length === 1 && amounts.length === 0)
        return parseCoinList(denoms[0]);
    if (denoms.length === amounts.length)
        return denoms.map((asset, index) => ({ asset, amount: amounts[index] }));
...
function isWasm(event) {
    return event.type === "wasm" || event.type.startsWith("wasm-") || event.type === "execute";
}
export function normalizeWasmEvent(event, context, contracts) {
    if (!isWasm(event))
        return undefined;
    ...
    if (contract === contracts.factoryAddress && ["create_pair", "pair_created", "create_pair_and_distribution_flows", "register"].includes(action)) {
        ...
        if (!pairAddress || !pairAddress.startsWith("juno1"))
            return undefined;
        return {
            ...context,
            kind: "pool_created",
