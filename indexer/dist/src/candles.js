export const SUPPORTED_CANDLE_INTERVALS = ["5m", "1h", "1d"];
const INTERVAL_MS = {
    "5m": 5 * 60 * 1000,
    "1h": 60 * 60 * 1000,
    "1d": 24 * 60 * 60 * 1000,
};
export function isCandleInterval(value) {
    return SUPPORTED_CANDLE_INTERVALS.includes(value);
}
export function bucketStartFor(blockTime, interval) {
    const date = blockTime instanceof Date ? blockTime : new Date(blockTime);
    if (Number.isNaN(date.getTime()))
        throw new Error(`invalid candle timestamp: ${blockTime}`);
...
export function aggregateSwapsToCandles(swaps, interval, decimals = {}) {
